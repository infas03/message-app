  ...
  return (
    <div className={`flex ${chatClassName} gap-2.5 w-full mb-4`}>
      <div className="flex flex-col w-fit max-w-[250px]">
        <div
          className={`px-4 py-1.5 border-gray-200 ${bubbleBgColor} text-white rounded-xl dark:bg-gray-700`}
        >
          <p className={`text-sm font-medium px-4 ${fromMe ? 'text-right' : 'text-left'}`}>{message?.message}</p>
        </div>
